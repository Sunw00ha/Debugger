...
bool handle_int_operations(enum OPERATORS op, struct RAM_VALUE lhs, struct RAM_VALUE rhs, struct RAM_VALUE* result, int line) {
  switch (op) {
    ...
    case OPERATOR_POWER:
      result->value_type = RAM_TYPE_INT;
      result->types.i = pow(lhs.types.i, rhs.types.i);
      break;
...
// implementation for input function call

// parameters: struct ELEMENT*, char*, struct RAM*, int
// returns: bool
// */
// bool handle_input_function(struct ELEMENT* parameter, char* var_name, struct RAM* memory, int line){
//   printf("hi from beginning of input\n");
//   // check------>
//   if (parameter == NULL || parameter->element_type != ELEMENT_STR_LITERAL) { // Input is always given a strliteral only
//     printf("input expects string literal only\n");
//     return false; // invalid input
//   }
//   // check done ------->
  
//   char* message = parameter->element_value; // Get the message to display to the user
//   printf("%s", message);
...
//   new_value.types.s = input_line;

//   if (!ram_write_cell_by_name(memory, new_value, var_name)) { // Write to memory to the provided variable
//     printf("failed to write input to memory\n");
//     return false;
//   }
  
//   printf("hi from end of input\n");
...
struct RAM_VALUE* handle_int_conversion(char* var_name, struct RAM* memory, int line) {
  ...
  int int_value = atoi(value->types.s);
...
parameters: struct VALUE*, struct RAM*, struct RAM_VALUE* (returned as a pointer parameter), int line
returns: bool
*/
...
parameters: struct FUNCTION_CALL, struct RAM*
returns: 
*/
...
bool execute_assignment(struct STMT* stmt, struct RAM* memory) {
  if (stmt->stmt_type == STMT_ASSIGNMENT) { // assume this is going to be true
    ...
    if (rhs->value_type == VALUE_FUNCTION_CALL) {
      ...
      // struct RAM_VALUE new_cell;
      // new_cell = *result;

      // new_value.value_type = new_cell.value_type;
      // new_value.types = new_cell.types;
    }
...
bool execute_while_loop(struct STMT* stmt, struct RAM* memory) {
  ...
  while(true) {
    ...
    if (!evaluate_expression(condition, memory, &condition_result, stmt->line)) {
      return false; // Stop execution if there’s a semantic error
    }
    ...
    while (current_body_stmt != NULL) {
      if (current_body_stmt->stmt_type == STMT_ASSIGNMENT) {
        if (!execute_assignment(current_body_stmt, memory)) {
          return false; // Stop if there’s an error in the body
        }
        current_body_stmt = current_body_stmt->types.assignment->next_stmt;

      } else if (current_body_stmt->stmt_type == STMT_FUNCTION_CALL) {
        if (!execute_function_call(current_body_stmt, memory)) {
          return false; // Stop if there’s an error in the body
        }
        ...
        if (!execute_while_loop(current_body_stmt, memory)) {
          return false; // Stop if there’s an error in the nested loop
        }
        current_body_stmt = current_body_stmt->types.while_loop->next_stmt;

      } else {
        printf("**SEMANTIC ERROR: invalid operand types (line %d) CHECK THIS AGAIN\n", stmt->line);
        return false;
      }
    }
  }
...
void execute(struct STMT* program, struct RAM* memory)
{
  ...
  while (stmt != NULL) {
    if (stmt->stmt_type == STMT_ASSIGNMENT) {
      ...
      if (execute_while_loop(stmt, memory) == false) {
        return; // stop we ran into an error
      };
...
/*
execute while loop calls on stmt and stmt calls on loop
this works but has issues with ordering the functions
...
bool execute_while_loop(struct STMT* stmt, struct RAM* memory, int og_line) {
  ...
  while(true) {
    ...
    struct STMT* current_body_stmt = body;
    // printf("current body stmt line: %i", current_body_stmt->line); //
    // printf("current body stmt line: %i", current_body_stmt->types.assignment);
    while (current_body_stmt != NULL) {
