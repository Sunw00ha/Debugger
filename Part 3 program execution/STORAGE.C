  ...
  if (unary_expr->element->element_type == ELEMENT_INT_LITERAL) {
    ...
    if (unary_expr->expr_type == UNARY_ADDRESS_OF){ // ADDING SUPPORT FOR & RETRIEVAL
      ...
      if (address == -1) { // if the function returns -1 or the address doesnt exist
        printf("**SEMANTIC ERROR: name '%s' is not defined (line %d)", var_name, line);
        return false;
      }
...
bool execute_assignment(struct STMT* stmt, struct RAM* memory) {
  if (stmt->stmt_type == STMT_ASSIGNMENT) { // assume this is going to be true
    ...
    if (isDeref == true) {
      ...
      printf("value_type: %i\n", pointer_value->types.i);


      // Case 1: Check if pointer variable exists
      if (pointer_value == NULL) {
        printf("**SEMANTIC ERROR: name '%s' is not defined (line %d)\n", var_name, stmt->line);
        return false;
      }
      ...
      struct RAM_VALUE* target_value = ram_read_cell_by_addr(memory, address);
      if (target_value == NULL) {
        printf("**SEMANTIC ERROR: '%s' contains invalid address (line %d)\n", var_name, stmt->line);
        return false;
      }

      printf("value_type for target_val: %d\n", target_value->value_type);
      printf("value_type for target_val: %i\n", target_value->types.i);
      target_value->value_type = RAM_TYPE_PTR;

      printf("POST CHANGE value_type for target_val: %d\n", target_value->value_type);
      printf("POST CHANGE value_type for target_val: %i\n", target_value->types.i);
      *target_value = return_val; // All is well perform the assignment
      return true;
    }
