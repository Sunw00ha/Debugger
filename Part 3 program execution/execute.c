...
functions to manage different data types, pointer arithmetic, 
and operations involving pointers (dereferencing and address-of).
...
bool handle_int_operations(enum OPERATORS op, struct RAM_VALUE lhs, struct RAM_VALUE rhs, struct RAM_VALUE* result, int line) {
  bool isRamTypePtr = false; 
  if (lhs.value_type == RAM_TYPE_PTR || rhs.value_type == RAM_TYPE_PTR) { // check if we need to change the type at the end to ptr
    isRamTypePtr = true;
  }
  ...
  if (isRamTypePtr == true) { // if the lhs or rhs was a pointer then set the type to pointer
...
bool handle_print(struct ELEMENT* parameter, struct RAM* memory, int line) {
  ...
  if (parameter == NULL) {
    ...
    if (value->value_type == RAM_TYPE_STR) {
      ...
      printf("%d\n", value->types.i); // Print the memory address
    } else {
...
bool retrieve_value(struct UNARY_EXPR* unary_expr, struct RAM* memory, struct RAM_VALUE* value, int line){
  ...
  if (unary_expr->expr_type == UNARY_ADDRESS_OF){ // ADDING SUPPORT FOR & RETRIEVAL
      // printf("entered here\n"); // debug
      char* var_name = unary_expr->element->element_value;

      // use the ram function to get the addr
      int address = ram_get_addr(memory, var_name);

      if (address == -1) { // if the function returns -1 or the address doesnt exist
        printf("**SEMANTIC ERROR: name '%s' is not defined (line %d)\n", var_name, line);
        return false;
      }

      // if the function does exist...
      value->value_type = RAM_TYPE_PTR; // Store the type as ptr
      value->types.i = address; // Store the address
      return true;
  } else if (unary_expr->expr_type == UNARY_PTR_DEREF) {
    char* var_name = unary_expr->element->element_value;

    // Get the value of the pointer
    struct RAM_VALUE* pointer_value = ram_read_cell_by_name(memory, var_name);

    // Case 1: check if the pointer variable exists
    if (pointer_value == NULL) {
      printf("**SEMANTIC ERROR: name '%s' is not defined (line %d)\n", var_name, line);
      return false;
    }

    // Case 2: check if the value is a pointer
    if (pointer_value->value_type != RAM_TYPE_PTR) {
      printf("**SEMANTIC ERROR: invalid operand types (line %d)\n", line);
      return false;
    }

    int address = pointer_value->types.i; // get the address

    struct RAM_VALUE* dereferenced_value = ram_read_cell_by_addr(memory, address);
    if (dereferenced_value == NULL) { // address is out of bounds
      printf("**SEMANTIC ERROR: '%s' contains invalid address (line %d)\n", var_name, line);
      return false;
    }

    // Copy the dereferenced value into the result
    *value = *dereferenced_value;
    return true;
  
  
  } else if (unary_expr->expr_type == UNARY_ELEMENT) {
...
bool execute_binary_expression(struct EXPR* expr, struct RAM* memory, struct RAM_VALUE* result, int line) {
  ...
  // next chunk is for checking with ptr types and call the appropriate function
  } else if (lhs_val.value_type == RAM_TYPE_PTR && rhs_val.value_type == RAM_TYPE_INT) {
    return handle_int_operations(expr->operator, lhs_val, rhs_val, result, line);
  } else if (lhs_val.value_type == RAM_TYPE_PTR && rhs_val.value_type == RAM_TYPE_REAL) {
    return handle_real_operations(expr->operator, lhs_val, rhs_val, result, line);
  } else if (lhs_val.value_type == RAM_TYPE_INT && rhs_val.value_type == RAM_TYPE_PTR) {
    return handle_int_operations(expr->operator, lhs_val, rhs_val, result, line);
  } else if (lhs_val.value_type == RAM_TYPE_REAL && rhs_val.value_type == RAM_TYPE_PTR) {
    return handle_real_operations(expr->operator, lhs_val, rhs_val, result, line);
...
bool evaluate_expression(struct EXPR* expression, struct RAM* memory, struct RAM_VALUE* result, int line) {
  ...
  // for unary deref
  else if (expression->lhs->expr_type == UNARY_PTR_DEREF) {
    struct RAM_VALUE temp_result;
    if (!retrieve_value(expression->lhs, memory, &temp_result, line)) {
      printf("issue with retrieve value in evaluate_expression function\n");
      return false;
    }
    *result = temp_result; // assign dereferenced value
    return true;
  }
...
bool execute_assignment(struct STMT* stmt, struct RAM* memory) {
  if (stmt->stmt_type == STMT_ASSIGNMENT) { // assume this is going to be true
    ...
    if (rhs->value_type == VALUE_EXPR) { // if it is a expression on the rhs...
      ...
      // Check if it's a dereference operation
      if (isDeref) {
        struct RAM_VALUE* pointer_value = ram_read_cell_by_name(memory, var_name);
        ...
        // Case 2: Check if variable is a pointer
        if (pointer_value->value_type != RAM_TYPE_PTR) {
          printf("**SEMANTIC ERROR: invalid operand types (line %d)\n", stmt->line);
          return false;
        }

        // Case 3: Validate pointer address
        int address = pointer_value->types.i;
        // pointer_value->value_type = RAM_TYPE_PTR;

        //evaluate, write by address
        ...
        if (!ram_write_cell_by_addr(memory, return_val, address)) {
          printf("**SEMANTIC ERROR: name '%s' is not defined (line %d)\n", var_name, stmt->line);
          return false;
        }
    ...
    if (rhs->value_type == VALUE_EXPR && rhs->types.expr->lhs->expr_type == UNARY_ADDRESS_OF) {
      ...
      if (!retrieve_value(rhs->types.expr->lhs, memory, &return_val, stmt->line)) { // call this function on the address of return val for consistency
        printf("failed to retireve val for &");
        return false; // the retrieval failed
      }
    }
